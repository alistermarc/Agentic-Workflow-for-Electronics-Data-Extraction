...
from nodes import load_and_split, extract_anchor, call_llm, parse_and_repair, finalize, save_full_state
...
def build_graph():
    ...
    # g.add_edge("anchor", "filter")
    # g.add_edge("filter", "llm")
    g.add_edge("anchor", "llm")
    g.add_edge("llm", "parse")
    g.add_conditional_edges(
        "parse",
        lambda s: "llm" if s["current_idx"] < len(s["chunks"]) else "final",
        {"llm": "llm", "final": "final"}
    )
